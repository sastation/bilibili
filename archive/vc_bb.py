...
def db_inital():
    '''初始化数据库，并将数据从原来文本文件中导入'''
    conn = sqlite3.connect('vc_sample.db')

    # please create table if db not exist
    conn.execute('create table bbvc (avnum vchar(25) primary key,'
    ...
    for line in fs:
        ...
        conn.execute('insert into bbvc'
                     ...
                     ' values(?,?,?,?,?);',
                     (cols[0], cols[1], cols[2], cols[3], cols[4].decode('utf-8')))
        i = i + 1
        if i % 10 == 0:
            conn.commit()
    fs.close()
